...
import type { MonthlyTransactions, TimeSeries } from '../types/models'
...
function AccountingPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const budgets = await accountingService.getBudgets(token, monthKey)
  ...
  const combinedTimeSeries =
    incomeTimeSeries?.data.map((item, idx) => ({
      date: item.date,
      income: item.amount,
      expense: expenseTimeSeries?.data[idx]?.amount ?? 0,
    })) ?? []

  const inferredBudgetItems = (): Array<{ flow_type: 'income' | 'expense'; category: string }> => {
    const base: Array<{ flow_type: 'income' | 'expense'; category: string }> = []
    const incomeCats = new Set((incomeTransactions?.transactions ?? []).map((txn) => txn.category))
    const expenseCats = new Set((expenseTransactions?.transactions ?? []).map((txn) => txn.category))
    incomeCats.forEach((category) => base.push({ flow_type: 'income', category }))
    expenseCats.forEach((category) => base.push({ flow_type: 'expense', category }))
    if (!base.length) {
      base.push({ flow_type: 'income', category: 'Uncategorized' })
      base.push({ flow_type: 'expense', category: 'Uncategorized' })
    }
    return base
  }
  ...
  const handleSaveBudget = async () => {
    ...
    try {
      const items = inferredBudgetItems().map((item) => ({
        flow_type: item.flow_type,
        category: item.category,
        amount: Number(budgetDraft[`${item.flow_type}:${item.category}`] ?? 0),
      }))
      await accountingService.saveBudgets(token, monthKey, items)
    } catch (error) {
      console.error('Error saving accounting budgets:', error)
    } finally {
  ...
  return (
    <section className="space-y-4">
        ...
        <article className="glass-panel p-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className={`font-display text-xl ${netIncome >= 0 ? 'text-green-600' : 'text-red-600'}`}>{formatMoney(netIncome)}</p>
            </div>
          </div>
        </article>
      </div>
              ...
              <tbody>
                {incomeTransactions?.transactions.map((txn) => (
                  <tr key={txn.id} className="border-b border-slate-100">
                    <td className="py-2 text-xs text-slate-500">{new Date(txn.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</td>
                    <td className="py-2 text-sm text-slate-900">{txn.description}</td>
                    <td className="py-2 text-xs text-slate-500">{txn.category}</td>
                    <td className="py-2 text-right text-sm font-semibold text-green-600">{formatMoney(txn.amount)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <tbody>
                {expenseTransactions?.transactions.map((txn) => (
                  <tr key={txn.id} className="border-b border-slate-100">
                    ...
                    <td className="py-2 text-sm text-slate-900">{txn.description}</td>
                    <td className="py-2 text-xs text-slate-500">{txn.category}</td>
                    <td className="py-2 text-right text-sm font-semibold text-red-600">{formatMoney(txn.amount)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="h-[350px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={combinedTimeSeries} margin={{ left: 20, right: 20, top: 10, bottom: 30 }}>
              ...
              <XAxis dataKey="date" tick={{ fill: '#64748b', fontSize: 11 }} angle={-45} textAnchor="end" height={60} />
              <YAxis tick={{ fill: '#64748b', fontSize: 12 }} />
              <Tooltip formatter={(value) => formatMoney(Number(value))} contentStyle={{ backgroundColor: 'white', border: '1px solid #e2e8f0', borderRadius: '8px' }} />
              <Legend />
              <Line type="monotone" dataKey="income" stroke="#10b981" strokeWidth={2} name="Income" dot={{ fill: '#10b981', r: 4 }} />
              <Line type="monotone" dataKey="expense" stroke="#ef4444" strokeWidth={2} name="Expenses" dot={{ fill: '#ef4444', r: 4 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </article>
      ...
      <article className="glass-panel p-5">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Budgeted Settings</h2>
            <p className="text-sm text-slate-500">Edit month budget values for {monthKey}</p>
          </div>
          <button
            type="button"
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {inferredBudgetItems().map((item) => {
            const key = `${item.flow_type}:${item.category}`
            return (
              <label key={key} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2 text-sm">
                <span className="text-slate-700">
                  {item.flow_type === 'income' ? 'Income' : 'Expense'} - {item.category}
                </span>
                <input
